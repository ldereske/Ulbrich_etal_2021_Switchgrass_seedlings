...
```{r, include=FALSE}
#How does the Chad suggestion of glmmTMB look
...
#Total germination
#total failed
fin_dataSG_seed_surv_trt_tot_seedling$fail_tot_num_germ=10-fin_dataSG_seed_surv_trt_tot_seedling$tot_num_germ


seed_germ_model_TMB= glmmTMB(cbind(tot_num_germ, fail_tot_num_germ)~precip*soil_root+(1|block), ziformula=~1,family=binomial, data= fin_dataSG_seed_surv_trt_tot_seedling)
qqPlot(resid(seed_germ_model_TMB))
hist(resid(seed_germ_model_TMB))
shapiro.test(resid(seed_germ_model_TMB))
#p-value =  3.741e-05
fixef(seed_germ_model_TMB)
plogis(-0.6436)
summary(seed_germ_model_TMB)

Anova(seed_germ_model_TMB,type = 3)


seed_germ_model_TMB_NB= glmmTMB(tot_num_germ~precip*soil_root+(1|block), ziformula=~1,family=nbinom2, data= fin_dataSG_seed_surv_trt_tot_seedling)
qqPlot(resid(seed_germ_model_TMB_NB))
hist(resid(seed_germ_model_TMB_NB))
shapiro.test(resid(seed_germ_model_TMB_NB))
#p-value = 7.978e-05

Anova(seed_germ_model_TMB_NB,type = 3)
summary(seed_germ_model_TMB_NB)

#let's try fitting a negative binomial GLM 

seed_germ_model_GLM_NB=glm.nb(tot_num_germ~precip*soil_root+block,data= fin_dataSG_seed_surv_trt_tot_seedling)
qqPlot(resid(seed_germ_model_GLM_NB))
hist(resid(seed_germ_model_GLM_NB))
shapiro.test(resid(seed_germ_model_GLM_NB))
#p-value = 0.0004203

Anova(seed_germ_model_GLM_NB,type = 3)
anova(seed_germ_model_GLM_NB)
summary(seed_germ_model_GLM_NB)
